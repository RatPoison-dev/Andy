...
let constructStatsBy = async (statsBy, client, message, args) => {
    ...
    embed.addField("KDR", `${kpd}%`, true)
    embed.addField("Won games", stats[`${statsBy}_won_games`], true)
    embed.addField("Lost games", stats[`${statsBy}_lost_games`], true)
    embed.addField("Total Games", stats[`${statsBy}_lost_games`] + stats[`${statsBy}_won_games`])
    embed.addField("Won", `${Math.floor(stats[`${statsBy}_won`])} :moneybag:`, true)
    embed.addField("Lost", `${Math.floor(stats[`${statsBy}_lost`])} :moneybag:`, true)
    return embed
}
